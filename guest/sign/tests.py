...
class LoginActionTest(TestCase):
	...
	def setUp(self):
		User.objects.create_user('admin','admin@mail.com','123456qjl')
		self.c = Client()
	...
	def test_login_action_success(self):
		'''登录成功'''
		test_data = {'username':'admin','password':'123456qjl'}
		response = self.c.post('/login_action/',data=test_data)
...
class EventManageTest(TestCase):
	'''发布会管理'''
	def setUp(self):
		Event.objects.create(id=2,name='xiaomi5',limit=2000,status=True,address='beijing',start_time=datetime(2017,8,10,14,0,0))
		self.c = Client()
...
class GuestManageTest(TestCase):
	...
	def setUp(self):
		Event.objects.create(id=1,name='xiaomi5',limit=2000,address='beijing',status=1,start_time=datetime(2017,8,10,14,0,0))
