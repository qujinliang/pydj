...
def add_event(request):
	...
	try:
		Event.objects.create(id=id,name=name,limit=limit,address=address,status=int(status),start_time=start_time)
...
def add_guest(request):
	id = request.POST.get('id','')     #关联发布会id
	...
	try:
		...
		return JsonResponse({'status':10026,'message':'the event guest phone mumber repeat'})
...
def user_sign(request):
	...
	result = Guest.objects.filter(phone = phoen)
	if not result:
	...
	if result:
		return JsonResponse({'stauts':10027,'message':'user has sign in'})
